structure Inferring :> INFERRING = struct
  ...
  fun substTypedTerm S (TypedTerm.VAR x) = TypedTerm.VAR x
    ...
    | substTypedTerm S (TypedTerm.LET (x, T, t, u)) = TypedTerm.LET (x, Type.subst S T, substTypedTerm S t, substTypedTerm S u)

  exception NotInScope of id
  exception Cyclic of id * Type.t
  exception Incompatible of Type.t * Type.t

  fun constraint_type fresh polyVars e (ImplicitTypedTerm.VAR x) =
    ...
    | constraint_type fresh polyVars e (ImplicitTypedTerm.APP (t, u)) = let
    ...
    | constraint_type fresh polyVars e (ImplicitTypedTerm.ABS (x, NONE, t)) = let
    ...
    | constraint_type fresh polyVars e (ImplicitTypedTerm.ABS (x, SOME T, t)) = let
        val e' = (x, T) :: e
        val (t', U, C) = constraint_type fresh polyVars e' t
      in
        (TypedTerm.ABS (x, T, t'), Type.ARR (T, U), C)
      end
    | constraint_type fresh polyVars e (ImplicitTypedTerm.LET (x, NONE, t, u)) = let
        val (t', T, C) = constraint_type fresh polyVars e t
        val S = unify C
        ...
        val (u', U, C') = constraint_type fresh polyVars' e' u
      in
        (TypedTerm.LET (x, T, t', u'), U, C @ C')
      end
    | constraint_type fresh polyVars e (ImplicitTypedTerm.LET (x, SOME T, t, u)) = let
        val (t', T', C) = constraint_type fresh polyVars e t
        val S = unify ((T, T') :: C)
        val (t', T, C) = (substTypedTerm S t', Type.subst S T, substConstraints S C)
        val monoVars = List.concat (map (Type.FV o #2) e)
