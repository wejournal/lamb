functor Compiling (ABI : ABI) :> COMPILING = struct
  type t = (KrivineMachine.code * id) list ref
  type env = id list
  ...
  fun compileInstr _ _ emitting _ (KrivineMachine.ACCESS i) =
    ...
    | compileInstr memo gensym emitting name (KrivineMachine.PUSH c) = let
        ...
        val name =
          case already of
            NONE =>
              (region name, value name ^ "_" ^ Int.toString (Gensym.gensym gensym))
          | SOME name =>
              name
      in
        Emitting.emitList
  ...
  fun compile memo gensym emitting E x c = let
    val name' = (region x, value x ^ "_" ^ Int.toString (Gensym.gensym gensym))
  in
    Emitting.emitList
      [ ".globl\t", value x, "\n", value x, ":\n"
      , "\tenter\t$64,\t$0\n" ]
